...
from utils.DO_Conv.do_conv_pytorch_1_10 import DOConv2d

def channel_shuffle(x, groups):
    # Standar Channel Shuffle dari paper ShuffleNet
...
class CBS(nn.Module):
    ...
    Ref: Figure 1 (YOLOv5 Architecture) & Figure 4 (MCFN Internal)
    """
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        super().__init__()
...
class DoConvCBS(nn.Module):
    """
    Wrapper agar DOConv2d bisa dipakai gaya YOLO (Conv + BN + SiLU).
    Digunakan di Backbone ShuffleNet_BD dan Neck L-CIFM.
    """
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        super().__init__()
        
        # Hitung padding otomatis jika tidak diberikan
        if p is None:
            p = k // 2 if isinstance(k, int) else [x // 2 for x in k]
            
        # PENTING: Gunakan DOConv2d (kode Anda), bukan nn.Conv2d
        # D_mul bisa diset default (None) atau disesuaikan
        self.conv = DOConv2d(c1, c2, kernel_size=k, stride=s, padding=p, groups=g, bias=False)
        
        # Standard BatchNorm & Activation
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act else nn.Identity()
...
class Shuffle_Block_BD(nn.Module):
    """
    ShuffleNet V2 Block dengan DoConv (BD).
    Ref: Figure 2a (Stride 1) & Figure 2b (Stride 2) Paper Utama.
    """
    def __init__(self, inp, oup, stride):
        super(Shuffle_Block_BD, self).__init__()
        
        if not (stride in [1, 2]):
            raise ValueError('stride must be 1 or 2')
        self.stride = stride

        # Hitung pembagian channel output
        # Di ShuffleNetV2, output channel biasanya dibagi 2 (setengah kiri, setengah kanan)
        branch_features = oup // 2
        
        # --- KASUS 1: STRIDE = 2 (Downsampling) - Figure 2b ---
        if self.stride == 2:
            # Cabang Kiri (Left Branch):
            # Figure 2b: 3x3 S=2 DoConv -> 1x1 Conv
            self.branch1 = nn.Sequential(
                # DoConv depthwise (groups=inp)
                DoConvCBS(inp, inp, k=3, s=2, p=1, g=inp, act=False),
                # Conv 1x1 standard
                CBS(inp, branch_features, k=1, s=1, act=True)
            )
            
            # Cabang Kanan (Right Branch):
            # Figure 2b: 1x1 Conv -> 3x3 S=2 DoConv -> 1x1 Conv
            self.branch2 = nn.Sequential(
                # 1x1 Conv
                CBS(inp, branch_features, k=1, s=1, act=True),
                # DoConv depthwise (groups=branch_features)
                DoConvCBS(branch_features, branch_features, k=3, s=2, p=1, g=branch_features, act=False),
                # 1x1 Conv
                CBS(branch_features, branch_features, k=1, s=1, act=True)
            )
            
        # --- KASUS 2: STRIDE = 1 (Basic Unit) - Figure 2a ---
        else:
            # Pada Stride=1, input di-split menjadi 2 channel (inp // 2).
            # Cabang Kiri: Identity (Tidak melakukan apa-apa) -> Jadi self.branch1 tidak perlu didefinisikan.
            
            # Cabang Kanan (Right Branch):
            # Figure 2a: 1x1 Conv -> 3x3 DoConv -> 1x1 Conv
            # Input channelnya adalah separuh dari total input (karena split)
            self.branch2 = nn.Sequential(
                # 1x1 Conv
                CBS(inp // 2, branch_features, k=1, s=1, act=True),
                # DoConv depthwise
                DoConvCBS(branch_features, branch_features, k=3, s=1, p=1, g=branch_features, act=False),
                # 1x1 Conv
                CBS(branch_features, branch_features, k=1, s=1, act=True)
            )
    ...
    def forward(self, x):
        if self.stride == 1:
            # Channel Split (Membagi input menjadi 2 bagian: x1, x2)
            # x1 lewat jalur bypass (identity), x2 masuk branch2
            x1, x2 = x.chunk(2, dim=1)
            
            # Gabungkan: x1 (asli) + branch2(x2)
            out = torch.cat((x1, self.branch2(x2)), dim=1)
        else:
            # Stride 2: Input disalin ke kedua cabang (tidak di-split)
            # Output channel akan berlipat ganda (karena concat branch1 dan branch2)
            out = torch.cat((self.branch1(x), self.branch2(x)), dim=1)

        # Terakhir: Channel Shuffle agar informasi tercampur
...
class MCFN(nn.Module):
    """
    Maxpool Cross-scale Fusion Node (M-CFN)
    Implementasi presisi berdasarkan Figure 4 di Paper 1.
    
    Arsitektur:
    1. Input Ci-1 (Resolusi Besar/Shallow) -> MaxPool (Stride 2)
    2. Input Ci   (Resolusi Target)        -> Identity
    3. Input Ci+1 (Resolusi Kecil/Deep)    -> Upsample (Factor 2)
    4. Concat ketiganya
    5. Bottleneck: CBS(1x1) untuk reduksi channel -> CBS(3x3) untuk ekstraksi fitur
    """
    def __init__(self, c_low, c_mid, c_high, c_out):
        """
        args:
            c_low: Channel input layer dangkal/besar (misal P3)
            c_mid: Channel input layer tengah/target (misal P4)
            c_high: Channel input layer dalam/kecil (misal P5)
            c_out: Channel output yang diinginkan
        """
        super().__init__()
        
        # Branch 1: Downsample layer resolusi besar (Ci-1)
        # Sesuai Paper 1: "Focus replaced by Maxpool stride 2"
        if c_low > 0:
            self.maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        else:
            self.maxpool = None # Penanda tidak ada input low
        
        # Branch 2: Upsample layer resolusi kecil (Ci+1)
        # Sesuai Paper 2: "Bilinear interpolation"
        if c_high > 0:
            self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        else:
            self.upsample = None # Penanda tidak ada input high
        
        # Hitung total channel setelah concat
        # PENTING: Kita harus tahu total channel input untuk layer Conv 1x1 berikutnya
        concat_channels = c_low + c_mid + c_high
        
        # Bottleneck Structure (Figure 4 Blue Box)
        # Layer 1: CBS k=1. Berfungsi mereduksi channel yang meledak akibat concat.
        # Kita kembalikan jumlah channel ke ukuran 'c_out' agar model tetap ringan.
        self.conv1 = CBS(concat_channels, c_out, k=1, s=1)

        # Layer 2: CBS k=3. Ekstraksi fitur utama.
        self.conv3 = CBS(c_out, c_out, k=3, s=1)

    def forward(self, x):
        """
        Input boleh 'None' jika tidak ada datanya.
        Contoh node pinggir: forward(None, p4, p5)
        """
        if isinstance(x, list):
            x_low, x_mid, x_high = x[0], x[1], x[2]

        features = []
        
        # Cek Input Low
        if self.maxpool is not None and x_low is not None:
            features.append(self.maxpool(x_low))
        elif self.maxpool is not None and x_low is None:
            # Safety check: Jika di init dibilang ada channel, tapi data kosong -> Error
            raise ValueError("MCFN Error: c_low > 0 di init, tapi x_low is None di forward!")
            
        # Input Mid (Selalu Ada)
        features.append(x_mid)
        
        # Cek Input High
        if self.upsample is not None and x_high is not None:
            features.append(self.upsample(x_high))
        elif self.upsample is not None and x_high is None:
            raise ValueError("MCFN Error: c_high > 0 di init, tapi x_high is None di forward!")
        
        # Gabung semua fitur yang tersedia
...
class EMA(nn.Module):
    ...
    Ref: Figure 5 in "Efficient Multi-Scale Attention Module with Cross-Spatial Learning"
    """
    def __init__(self, channels, factor=32):
        super(EMA, self).__init__()
        self.groups = factor
        while self.groups > 1 and channels % self.groups != 0:
            self.groups //= 2 # Turunkan pangkat 2 (32 -> 16 -> 8 -> 4 -> 2 -> 1)

        # Safety net terakhir: Jika 1 pun gagal (mustahil sih), set ke 1.
        if self.groups < 1: 
            self.groups = 1
        
        # Safety Check: Pastikan channel bisa dibagi habis dengan jumlah group
        assert channels % self.groups == 0, f"Channels {channels} tidak bisa dibagi habis dengan groups {self.groups}"
        ...
        # GroupNorm (Sesuai Diagram kotak putus-putus)
        self.gn = nn.GroupNorm(channels // self.groups, channels // self.groups)
        
        # Conv 1x1 untuk Coordinate Attention Branch
        self.conv1x1 = nn.Conv2d(channels // self.groups, channels // self.groups, kernel_size=1, stride=1, padding=0)
        
        # Conv 3x3 untuk Local Branch
    ...
    def forward(self, x):
        ...
        # 2. Coordinate Attention Branch (Left)
        x_h = self.pool_h(group_x)
        x_w = self.pool_w(group_x).permute(0, 1, 3, 2) # Permute agar bisa dicat
        
        hw = self.conv1x1(torch.cat([x_h, x_w], dim=2))
        x_h, x_w = torch.split(hw, [h, w], dim=2)
        
        # Re-weighting awal sebelum masuk GroupNorm
        x1 = self.gn(group_x * x_h.sigmoid() * x_w.permute(0, 1, 3, 2).sigmoid())
        
        # 3. Local 3x3 Branch (Right)
        x2 = self.conv3x3(group_x)
        
        # 4. Cross-Spatial Learning (Matmul Silang)
        # Menghitung vektor atensi global dari masing-masing cabang
        x11 = self.softmax(self.agp(x1).reshape(b * self.groups, -1, 1).permute(0, 2, 1))
        x12 = x2.reshape(b * self.groups, c // self.groups, -1) # Flatten H,W
        
        x21 = self.softmax(self.agp(x2).reshape(b * self.groups, -1, 1).permute(0, 2, 1))
        x22 = x1.reshape(b * self.groups, c // self.groups, -1) # Flatten H,W
        
        # Matmul: [B*G, 1, C//G] x [B*G, C//G, HW] -> [B*G, 1, HW]
        # Ini menghasilkan peta atensi spasial
        weights = (torch.matmul(x11, x12) + torch.matmul(x21, x22)).reshape(b * self.groups, 1, h, w)
        
        # 5. Final Re-weighting & Reshape
        return (group_x * weights.sigmoid()).reshape(b, c, h, w)


class ContextBottleneck(nn.Module):
    """
    Bagian 'Bottleneck' di dalam Figure 6 L-CIFM.
    Memiliki dua cabang (Left & Right) yang kemudian dijumlahkan (Add).
    """
    def __init__(self, c_in, c_out):
        super().__init__()
        # Pastikan c_in sama dengan c_out untuk operasi penjumlahan (Add)
        assert c_in == c_out, "ContextBottleneck input/output channels must be same for Add operation"
        
        # Branch Kiri (Left Branch): 
        # Sesuai Paper: "Left branch uses a 3x3 DoConv ... Then a 1x1 standard convolution"
        self.branch_left = nn.Sequential(
            DoConvCBS(c_in, c_out, k=3, s=1, g=1), # DoConv 3x3
            CBS(c_out, c_out, k=1, s=1)            # Conv 1x1
        )
        
        # Branch Kanan (Right Branch):
        # Sesuai Paper: "Right branch first uses a 1x1 standard convolution ... then enriches local features through a 3x3 DoConv"
        self.branch_right = nn.Sequential(
            CBS(c_in, c_out, k=1, s=1),            # Conv 1x1
            DoConvCBS(c_out, c_out, k=3, s=1, g=1) # DoConv 3x3
        )

    def forward(self, x):
        # Contextual Fusion Feature obtained by adding (+)
        return self.branch_left(x) + self.branch_right(x)


class LCIFM(nn.Module):
    """
    Lightweight Contextual Information Fusion Module (L-CIFM).
    Ref: Figure 6 in 'Research on Fatigued-Driving Detection Method...'
    
    Digunakan untuk menggantikan modul C3 di Neck.
    """
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        """
        Args:
            c1: Input channels
            c2: Output channels
            n:  Number of repeats (Di paper Figure 6 hanya 1 blok, tapi kita beri opsi n untuk kompatibilitas YOLO)
            e:  Expansion factor (biasanya 0.5 di YOLOv5)
        """
        super().__init__()
        c_hidden = int(c2 * e)  # Hidden channels
        
        # 1. Preprocessing (Input -> Conv 1x1)
        # Sesuai diagram: Input masuk ke kotak kuning "CBS" di awal
        self.cv1 = CBS(c1, c_hidden, k=1, s=1)
        
        # 2. Jalur Bawah: Attention Branch (EMA)
        # Sesuai diagram: Panah bawah masuk ke blok EMA
        self.ema = EMA(c_hidden, factor=32) # factor bisa disesuaikan, paper EMA menyarankan 32
        
        # 3. Jalur Atas: Context Branch (Bottleneck)
        # Sesuai diagram: Panah lurus masuk ke blok Bottleneck
        # Jika n > 1, kita tumpuk bottleneck-nya (meski di paper cuma gambar 1)
        self.context_layers = nn.Sequential(*(
            ContextBottleneck(c_hidden, c_hidden) for _ in range(n)
        ))
        
        # 4. Final Fusion (Concatenation -> Conv 1x1)
        # Input ke cv2 adalah gabungan dari Context (c_hidden) dan Attention (c_hidden)
        # Jadi input channelnya = c_hidden * 2
        self.cv2 = CBS(c_hidden * 2, c2, k=1, s=1)

    def forward(self, x):
        # Tahap 1: Preprocessing
        x_hidden = self.cv1(x)
        
        # Tahap 2: Jalur Atas (Context)
        x_ctx = self.context_layers(x_hidden)
        
        # Tahap 3: Jalur Bawah (Attention)
        # "concatenated with the upper-level information enhanced by EMA"
        x_att = self.ema(x_hidden)
        
        # Tahap 4: Concatenation (Cat)
        x_cat = torch.cat([x_ctx, x_att], dim=1)
        
        # Tahap 5: Output (Equation 1 & Diagram Output)
        out = self.cv2(x_cat)
